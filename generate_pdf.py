...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .cover .flag-item:nth-child(3) svg { animation-delay: 1.5s; }
  ...
  @keyframes flagWave {
    ...
    25% { transform: perspective(400px) rotateY(3deg) rotateX(-1deg); }
    50% { transform: perspective(400px) rotateY(0deg) rotateX(1deg); }
    75% { transform: perspective(400px) rotateY(-3deg) rotateX(-1deg); }
  }
...
@media (max-width: 768px) {
  ...
  /* Mobile sidebar: full width bottom sheet */
  .nav-sidebar { width: 100% !important; height: 75% !important; top: auto !important; bottom: 0 !important; left: 0 !important; border-radius: 20px 20px 0 0; transform: translateY(100%) !important; padding: 16px !important; }
...
(function() {
  ...
  function initNav() {
    ...
    function createSidebar() {
      ...
      sidebar.style.cssText = 'position:fixed;top:0;left:0;width:280px;height:100%;background:' + bgColor + ';z-index:2000;box-shadow:2px 0 24px rgba(0,0,0,0.2);overflow-y:auto;padding:20px;transform:translateX(-100%);transition:transform 0.3s;';
      // Close button
      ...
      setTimeout(function() { if (sidebar) { sidebar.classList.add('open'); sidebar.style.transform = 'translateX(0)'; } }, 10);
      // Observe dark mode
